  ...
  return (
    <div className="my-4">
      <div className={`flex  ml-1 md:mb-3 rounded-md items-start text-sm`}>
        <span
          className="px-2 py-1 tc rounded-xl w-1/3"
          style={{ backgroundColor: renderColor(label) }}
        >
          {label}
        </span>
      </div>
      <div className="flex justify-center items-center">
        <div className="w-2/3 px-3 ">
          <Slider
            railStyle={{ backgroundColor: "#FCFCFC" }}
            value={value}
            max={max}
        ...
        <input
          ...
          className="w-1/6 tc mx-4"
          value={value}
        ></input>
        <div className="text-sm flex-1 justify-around  sm:flex ">
          <Button
            className="mb-1 min-w-0 md:mb-0"
            size="icon-sm"
            variant="ghost"
            onClick={() => {
...
export const ScoreCalculator: React.FC<Props> = ({
  rate,
  score,
  combo,
  haveScore,
  ...
  const calculateTotalScore = (scoreDetails: number[]) => {
    return Math.trunc(
      ((scoreDetails[0] * 1.01 + scoreDetails[1] + scoreDetails[2] * 0.5) /
        combo) *
        1000000,
    );
  };
  ...
  return (
    <div className="">
      ...
      <div className="w-full ">
        <SdSlider
          value={scoreDetails[0]}
          max={combo}
          onChange={(v) => {
            onChange(v as number, 0);
          }}
          label="CRITICAL JUSTICE"
        />
        <SdSlider
          value={scoreDetails[1]}
          max={combo}
          onChange={(v) => {
            onChange(v as number, 1);
          }}
          label="JUSTICE"
        />
        <SdSlider
          value={scoreDetails[2]}
          max={combo}
          onChange={(v) => {
            onChange(v as number, 2);
          }}
          label="GOOD"
        />
        <SdSlider
          value={scoreDetails[3]}
          max={combo}
          onChange={(v) => {
            onChange(v as number, 3);
          }}
          label="MISS"
        />
      </div>
